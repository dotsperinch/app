ID3v2 = {
	parseStream : function(stream, onComplete) {
		...
		// from:
		// http://bitbucket.org/moumar/ruby-mp3info/src/tip/lib/mp3info/id3v2.rb
		...
		// pulled from http://www.id3.org/id3v2-00 and changed with a simple
		// replace
		...
		function parseDuration(ms) {
			var msec = parseInt(cleanText(ms)); // leading nulls screw up
												// parseInt
		...
		function parseImage(str) {
			...
			var TextPictureType = PICTURE_TYPES[PictureType.toString(16)
					.toUpperCase()];
			str = str.substr(1);
			...
			if (tag.revision < 3) {
				read(3, function(frame_id) {
					...
					if (/[A-Z0-9]{3}/.test(frame_id)) {
						var new_frame_id = TAG_MAPPING_2_2_to_2_3[frame_id
								.substr(0, 3)];
				...
				read(2, function(s, flags) {
					...
					read(intsize, function(s, a) {
						if (typeof TAG_HANDLERS[frame_id] == 'function') {
							...
							tag[TAGS[frame_id]] = (tag[TAGS[frame_id]] || '')
									+ cleanText(s)
						} else {
	...
	parseURL : function(url, onComplete) {
		...
		var pos = 0, bits_required = 0, handle = function() {
		}, maxdata = Infinity;
...
function guessSong(n) {
	var parts = unescape(n).replace(/^.*(music|desktop|document|video|home)/gi,
			'').replace(/\[[^\]]+\]/g, '').replace(/_/g, ' ').replace(
			/[0-9]+\s*(kbps|\-)/, '').split(/[\/]/).reverse();

	var name = parts[0].replace(/\.(mp3|ogg|flac)/, '').replace(/^\s|\s$/g, '')
			.replace(/^\d+\s*/, '');
	...
	if (/\-/.test(name)) {
		...
		if (parts[2]) {
			var artist = parts[2].replace(/^\s|\s$/g, '')
					.replace(/^\d+\s*/, '');
		}
	}
	if (/\-/.test(album)) {
